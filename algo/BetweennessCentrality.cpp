...
std::vector<Vertex> findCentralAirports(int numAirports, Graph G) {
    ...
    for (size_t i = 0; i < vertices.size(); i++) {
        for (size_t j = 0; j < vertices.size(); j++) {
            if (i != j) {
                std::vector<Vertex> shortestPath = BFS.findShortestPath(vertices[i], vertices[j]); //Returns vector of nodes in shortest path between 2 given nodes, not including the given start and end. Empty if no path.
    ...
    //Remove top numAirports vertices.
    ...
    //centralAirports is now a vector with the top airports to be removed
